...
nb <- NbClust(sar[2:7], distance = "euclidean", min.nc = 2,
              max.nc = 10, method = "kmeans") #2 or 3 Clusters

fviz_nbclust(sar[2:7], kmeans, method = "wss") +
...
km2 <- kmeans(sar[2:7], 2, nstart = 25)
print(km2)
fviz_cluster(km2, data = sar[2:7],
             geom = "point",
             ellipse.type = "convex", 
             ggtheme = theme_bw())

km3 <- kmeans(sar[2:7], 3, nstart = 25)
print(km3)
fviz_cluster(km3, data = sar[2:7],
             geom = "point",
             ellipse.type = "convex", 
             ggtheme = theme_bw())
...
sil <- silhouette(km2$cluster, dist(sar[2:7]))
fviz_silhouette(sil)

sil <- silhouette(km3$cluster, dist(sar[2:7]))
fviz_silhouette(sil)
...
#Visualizing results
p <- ggparcoord(data = clusters, columns = c(2:7), groupColumn = "cluster") + labs(x = "Type", y = "value (in standard-deviation units)", title = "Clustering")
ggplotly(p)
...
scale <- data.frame(scale(clusters[2:7]))
...
hc.res <- eclust(sar[,2:7], "hclust", k = 3, hc_metric = "euclidean", 
